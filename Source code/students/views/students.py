...
class QuizListView(ListView):
    ...
    def get_queryset(self):
        ...
        student_interests = Course.objects.filter(students__in=[self.request.user])
        if(student.count()==0) : return Quiz.objects.filter(course__in=student_interests).annotate(question_count=Count('questions', distinct=True)).filter(question_count__gt=0)
        taken_quizzes = TakenQuiz.objects.filter(student__in=student)
        queryset =  Quiz.objects.filter(course__in=student_interests).exclude(taken_quizzes__in=taken_quizzes).annotate(question_count=Count('questions', distinct=True)).filter(question_count__gt=0)
        return queryset
...
class TakenQuizListView(ListView):
    ...
    def get_queryset(self):
        student = Student.objects.filter(user=self.request.user)
        taken_quizzes = TakenQuiz.objects.filter(student__in=student)
        queryset = taken_quizzes.select_related('quiz', 'quiz__course').order_by('quiz__name')
        return queryset
